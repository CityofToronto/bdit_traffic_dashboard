...
CREATE OR REPLACE VIEW data_analysis.gardiner_weeks AS
 SELECT a.dt::date AS week,
   ...
   FROM ( SELECT generate_series('2019-08-01 00:00:00'::timestamp without time zone, '2019-12-01 00:00:00'::timestamp without time zone, '7 days'::interval) AS dt) a
...
CREATE OR REPLACE VIEW data_analysis.gardiner_months AS
 SELECT a.dt::date AS month,
   ...
   FROM ( SELECT generate_series('2019-08-01 00:00:00'::timestamp without time zone, '2019-12-01 00:00:00'::timestamp without time zone, '1 mon'::interval) AS dt) a
  ORDER BY (row_number() OVER ()) DESC;
